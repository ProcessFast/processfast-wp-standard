...
class Main extends \TCB\Integrations\WooCommerce\Main {
	...
	public static function include_elements() {
		...
		if (
			...
			$theme_elements     = \Thrive_Architect::get_architect_theme_elements( static::INTEGRATION_PATH . 'elements' );
			switch ( $secondary_template ) {
				case self::CART_TEMPLATE:
				case self::CHECKOUT_TEMPLATE:
					static::$elements = array_merge(
						[ "$secondary_template-template" => $theme_elements[ "$secondary_template-template" ] ],
						\Thrive_Architect::get_architect_theme_elements( static::INTEGRATION_PATH . "sub-elements/$secondary_template" )
					);
					break;
				case self::POST_TYPE:
					if ( $primary_template === THRIVE_ARCHIVE_TEMPLATE ) {
						static::$elements = [ 'shop-template' => $theme_elements['shop-template'] ];
					} else {
						static::$elements = array_merge(
							[ "$secondary_template-template" => $theme_elements[ "$secondary_template-template" ] ],
							\Thrive_Architect::get_architect_theme_elements( static::INTEGRATION_PATH . "sub-elements/$secondary_template" )
				...
				case self::ACCOUNT_TEMPLATE:
					static::$elements = [ "$secondary_template-template" => $theme_elements[ "$secondary_template-template" ] ];
					break;
				default:
					break;
			}
